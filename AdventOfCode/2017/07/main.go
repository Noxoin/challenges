...
func solve(in []string) string {
	...
	for _, v := range in {
		s := strings.Split(v, " ")
		weight, _ := strconv.Atoi(s[1][1:len(s[1])-1])
		mains = append(mains, &disc{ name: s[0], weight: weight})
		if len(s) > 3 {
			s = s[3:]
			for _, t := range s {
...
func solve2(in []string) int {
	mains := make(map[string]*disc)
	stacks := make(map[string][]string)
	for _, v := range in {
		s := strings.Split(v, " ")
		weight, _ := strconv.Atoi(s[1][1:len(s[1])-1])
		mains[s[0]] = &disc{ name: s[0], weight: weight}
		if len(s) > 3 {
			n := s[0]
			s = s[3:]
			stacks[n] = make([]string, 0)
			for _, t := range s {
				name := strings.Replace(t, ",", "", -1)
				stacks[n] = append(stacks[n], name)
			}
		}
	}
	root := solve(in)
	_, y := dfp(mains, stacks, root)
	return y
}

func dfp(d map[string]*disc, s map[string][]string, node string) (int, int) {
	if s[node] == nil {
		return d[node].weight, 0
	}
	w := make(map[int]int)
	...
	diff := odd - common
	return 0, d[mem[odd]].weight - diff
}
...
lfkqyf (237)`
	fmt.Println(solve(strings.Split(in, "\n")))
	fmt.Println(solve2(strings.Split(in, "\n")))
}
